...
class OneMinuteData(Model):
    ...
    def __vars__():
        return [
            'id', 'pm25', 'pm10', 'temperature', 'pressure', 'humidity', 'time'
        ]
...
class Plotter:
    ...
    def plot_minutes(self, data: list,
        ...
        {converted[key].append(sub[key]) for sub in data for key in sub}
        convertedData = converted[value]
        _time = converted['time']
        # count how much points we have to know time-step
        labels_count = len(convertedData)
        if labels_count > 100:
    ...
    def plot_day(
        ...
        self.isValueCorrect(value)
        y1 = [
            data['morning']['min'], data['day']['min'], data['evening']['min'],
            data['night']['min']
        ]
        y2 = [
            data['morning']['max'], data['day']['max'], data['evening']['max'],
            data['night']['max']
        ]
    ...
    def plot_month(
        ...
        # turn list of dicts to dict of lists
        {converted[key].append(sub[key]) for sub in data for key in sub}
        # converted == {'date': [date1, date2, etc],
        # 'min': [min1, min2, etc], 'max': [max1, max2, etc]}
        min_line, = plt.plot(converted['date'],
                             converted['min'],
                             marker='.',
                             color='blue',
                             label='Минимум')
        max_line, = plt.plot(converted['date'],
                             converted['max'],
                             marker='.',
                             color='orange',
                             label='Максимум')
    ...
    def valueToStrWithUnits(value: str):
        ...
        if result is None:
            logging.error(
                f'Can not convert value ({value}) to string with units!')
